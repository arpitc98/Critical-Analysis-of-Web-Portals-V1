...
try:
    ...
    sys.exit(0)#checking if url is opening or not

forms = br.forms() #Finds all the forms present in webpage

headers = str(urlopen(url).headers).lower() #Fetches headers of webpage and converting them to lower
...
soup =  BeautifulSoup(data, 'lxml') #Parses the response with beuatiful soup
...
def brute(username, passwd, menu, option, name, form_number):
    ...
    return ( (unameexe+passexe) / (unamefile+passfile))
    #quit()
